...
| `MCPX-04` | Raw HTTP가 Host 421, Origin 403, 무인증/중복 auth 401, 잘못되거나 중복된 media type 400, 누락·이전·미지원·중복 protocol version 400, 1 MiB 초과 413과 bounded malformed JSON을 확인한다. Tool validation은 oversized SQL, extra field와 type coercion 입력을 반사하지 않는다. | PASS |
...
| `MCPX-08` | 실제 Uvicorn socket에서 current-version client call 취소가 client session 종료 전에 hanging executor를 취소하고 같은 client의 후속 tool call이 성공한다. 이전 initialize handshake는 parent boundary에서 fail-closed한다. | PASS |

## Executed Evidence

```text
uv run pytest
  PASS (220 passed, 30 deselected)

./scripts/verify-container.sh
  PASS

Codex CLI 0.149.0 + project feature mcp_2026_07_28=true
  PASS (startup error 없음; /mcp에서 get_context, list_sources, query 확인)

uv run pytest -m 'mcp_server and not soak' -s
  PASS (9 passed in 15.66s)
  quality cases 16; verified contracts 9
  same-client concurrent queries 24 (0.969s; max queue 496ms; max execution 411ms)
  independent sessions 8 (0.660s)
  active development holders 2
  holder outcomes QUERY_TIMEOUT / QUERY_TIMEOUT (5.089s / 5.111s)
  third development outcome QUERY_OVERLOADED (1.046s)
  concurrent market control 1,200 rows counted (0.076s)
  development recovery 600 rows counted (0.047s)

uv run pytest -m integration
  PASS (18 passed in 25.24s; includes current MCP disconnect cancellation)
...
이 감사에서 남았던 multi-replica session/resource 항목은
[multi-replica soak audit](2026-08-23-mcp-multi-replica-soak.md)에서 완료했다. 비용 귀속과
workflow trace는 [active development TODO](../development-todo.md)의 `COST-*`, `TRACE-*`로
관리한다. 이전 MCP handshake/version과 legacy cancellation은 지원 대상이나 backlog가 아니다.

현재 production 계약 안에서는 실제 Docker MCP의 자동 검증, 입력 비노출, 비용 포화와 current
