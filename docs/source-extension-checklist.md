...
| 기존 profile로 비용 상한을 만족하지 못함 | 별도 review 후 budget profile을 변경한다. Budget 파일은 startup 설정이므로 현재는 restart가 필요하고, 새 metadata revision에서 L2 verified query를 다시 승인한다. |
