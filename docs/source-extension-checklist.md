...
view, versioned manifest, 기존 budget profile과 verified query로 표현한다.
...
- [ ] Reader를 `NOSUPERUSER`, `NOCREATEDB`, `NOCREATEROLE`, `NOINHERIT`,
  `NOREPLICATION`, `NOBYPASSRLS`, workload와 pool에 맞는 유한한 양수 connection limit,
  default read-only, statement/transaction/lock timeout과 temp/work memory 상한으로 고정한다.
- [ ] 기존 workload에 맞는 budget profile을 선택하고 실제 대표 query의 plan cost, rows,
  node 수, 실행 시간과 결과 byte를 측정한다.
- [ ] Source-scoped secret 이름과 L0 manifest를 준비해 operator API에서 stage/publish하고
  generation과 metadata revision을 기록한다. 같은 `source_id`는 host, port, database,
  user 또는 TLS mode가 다른 endpoint로 다시 묶지 않는다.
- [ ] 누가 source를 볼지 명시한다. `all_sources: true` caller는 새 source를 즉시 보지만,
  제한 caller는 `allowed_sources` 정책 변경과 현재는 service restart가 필요하다.
...
| 기존 profile로 비용 상한을 만족하지 못함 | 별도 review 후 budget profile을 변경한다. Budget 파일은 startup 설정이므로 현재는 restart가 필요하다. |
...
Gateway가 강제하는 것은 caller/source authorization, metadata revision, SQL AST와 resolved
