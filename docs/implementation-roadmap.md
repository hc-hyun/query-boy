...
| `MOD-03` | Startup cleanup과 다섯 boundary debt의 용어, 객관식 선택지, 영향·불변조건·승인 형식을 이해 문서로 고정했다. | [module boundary decision guide](module-boundary-decision-guide.md), [`test_documentation.py`](../tests/test_documentation.py) |
...
| `PRE-REPO-SPLIT-20260826` | Control administration·persistence·secret·concrete metadata store와 managed Runtime/Delivery adapter를 same-repository `query_man.managed` package로 격리했다. Static runtime은 managed 구현·13개 source-admin route·usage recorder를 조립하지 않고, base fixture/CI는 current 두 source만 사용하며 managed acceptance는 별도 project/container/volume에서 실행한다. 기존 managed API, Control history/data와 lifecycle은 보존했으며 실제 repository/service 분리는 수행하지 않았다. | [ADR 0018](decisions/0018-module-ownership-and-contract-governance.md), [ADR 0025](decisions/0025-static-non-rls-first-launch.md), [module index](modules/README.md), [`test_server.py`](../tests/test_server.py), [`test_managed_http.py`](../tests/test_managed_http.py), [`test_source_store.py`](../tests/test_source_store.py), [CI workflow](../.github/workflows/ci.yml) |
| `PHYSICAL-MODULES-20260827` | Static implementation을 Source Catalog, Metadata, Guarded Query, Delivery, Runtime, Assurance owner package로 물리적 분리하고 owner별 model과 Delivery surface/Runtime composition을 각 leaf로 나눴다. Marker-only `__init__`, direct leaf import, old flat shim 없음을 적용했고 root `errors.py`만 interface-only shared artifact로 유지했다. Interface, wire/persisted format, policy/revision, lifecycle과 ADR 0025 launch scope는 보존했으며 같은 repository·wheel·process를 유지했다. | [ADR 0026](decisions/0026-physical-module-packages.md), [module index](modules/README.md), [`source_catalog/registry.py`](../src/query_man/source_catalog/registry.py), [`metadata/service.py`](../src/query_man/metadata/service.py), [`guarded_query/query.py`](../src/query_man/guarded_query/query.py), [`delivery/app.py`](../src/query_man/delivery/app.py), [`runtime/composition.py`](../src/query_man/runtime/composition.py), [`assurance/cli.py`](../src/query_man/assurance/cli.py), [`test_documentation.py`](../tests/test_documentation.py) |
| `OBS-01` | Server-side expiring consent receipt가 있는 authorized question과 literal-free SQL만 최대 7일 AES-GCM SQLite에 저장하고, HMAC subject, bounded queue/daily byte budget, fail-open lifecycle와 offline receipt purge를 고정했다. 일반 HTTP/MCP wire, SQL policy/revision과 DB-native 비용·workflow trace 범위는 유지했다. | [ADR 0027](decisions/0027-consent-gated-diagnostic-capture.md), [Operations](operations.md#consent-gated-diagnostic-capture), [`test_diagnostic_capture.py`](../tests/test_diagnostic_capture.py), [`test_http.py`](../tests/test_http.py) |
| `OPS-09` | `qm` interactive/one-shot shell에 빈 입력 guide, Tab completion, bounded status/log/diag/source 조회와 expected-state/receipt를 보존한 guided mutation을 추가했다. Source는 기존 managed HTTP API를 소비하고 local Compose log와 encrypted diagnostic helper를 별도 provider로 유지했다. | [ADR 0028](decisions/0028-interactive-operator-shell.md), [Operations](operations.md#interactive-operator-shell), [`operator_shell.py`](../src/query_man/runtime/operator_shell.py), [`test_operator_shell.py`](../tests/test_operator_shell.py) |
| `QB-YAML-SOURCE-AUTHORITY-20260829` | Git-reviewed source/verified-query/budget YAML을 단일 authority로 확정하고 managed package, Control DB migration/store, admin mutation, hot reload, replica/usage reporter와 recovery fixture를 제거했다. Retired environment setting은 값을 노출하지 않고 fail-closed하며, live Control DB data 작업은 수행하지 않았다. | [ADR 0030](decisions/0030-git-reviewed-yaml-source-authority.md), [verification](verification/2026-08-29-yaml-source-authority.md), [Source management tombstone](source-management-plane.md), [`query-man-source-onboarding`](../skills/query-man-source-onboarding/SKILL.md) |
