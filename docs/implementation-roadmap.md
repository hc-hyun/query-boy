...
| M14.5 Lossless Scalar And Reader Formatting | `ENC-*` | Calendar interval/nested JSON numeric의 손실과 empty multirange 오인을 닫고 scalar SQL 의미·decode를 role default와 무관하게 고정한다. |
...
M1부터 M13, `TIME-01`~`TIME-02`와 별도 assurance `DBEDGE-01`~`DBEDGE-02`는 완료됐다.
M14.5의 `ENC-*` 결정·구현과 M14 production 전환 `TIME-03`은 active이며 M15와 M16은 각각
정확한 계약을 다시 승인받아야 한다. 새로운 기능은 기존 완료 ID나 설명을 소급 변경하지 않고
별도 roadmap 항목과 검증 가능한 exit condition을 추가한다.
