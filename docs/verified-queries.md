...
Status: Active bootstrap/CI executable dataset; managed production authority is Control DB

[`config/verified-queries.yaml`](../config/verified-queries.yaml)은 bootstrap/CI MVP golden question의
versioned executable verified-query dataset이다. Managed production runtime은 이 file을 열지 않고
...
이 명령은 bootstrap fixture 회귀용이며 managed production verified-query inventory를 검증하거나
