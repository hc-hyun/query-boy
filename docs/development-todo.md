...
| Approval gate | [Proposed ADR 0024](decisions/0024-rls-policy-drift-attestation.md)의 `RLS-01-A`는 ordinary invoker-view/table, ENABLE+FORCE, 정확히 하나의 PUBLIC 또는 exact-reader SELECT tenant equality, exact `_RETURN`/dependency와 custom/partition 거부, canonical bound, lock-first query order, snapshot/revision v2, error/no-stale, generation recheck, existing-operator tenant mapping, standalone v2 또는 combined direct-v3 cutover/rollback을 묶은 implementation-ready 제안이다. 이 persisted/Python/policy/lifecycle 계약과 residual trusted-admin 경계를 정확히 승인한 뒤 구현한다. 일반적인 진행/승인이나 ID만으로는 선택 승인이 아니다. B는 별도 exact restatement가 필요하고 C는 production-blocking defer다. |
