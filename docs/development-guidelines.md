...
| Internal interface 또는 wire·format·policy·lifecycle·procedure | [Module Interface와 승인 대상 변경](#module-interface와-승인-대상-변경) |
...
Query Man의 module boundary는 독립 배포나 임의 구현 교체 단위를 늘리기 위한 것이 아니라, 사람과
agent가 한 작업에 필요한 코드만 빠르게 찾고 이해하도록 만드는 **독립적인 이해·변경 단위**다.
Owner가 보장하는 동작을 보존한 module 내부 변경은 다른 module의 구현 변경으로 불필요하게 번지지
않아야 한다. Package graph를 기계적인 DAG로 만들거나 모든 내부 구현에 대체 가능성을 요구하지 않는다.
...
- 새 경계보다 삭제, 기존 module 재사용 또는 가까운 위치의 평범한 코드로 충분하면 거기서 멈춘다.
  경계 변경이 아래 별도 승인 대상 의미에 닿으면 승인 절차를 먼저 따른다.

## Module-Scoped Development

Query Man은 하나의 repository, wheel과 deployable process를 유지하는 modular monolith다. Core는
owner별 여섯 physical package로 나뉜다. 논리 module의 owner, 현재 leaf-file mapping, 허용 dependency와
중요한 entrypoint는
...
- Coordinating agent는 병렬 작업을 시작하기 전에 각 agent에게 primary module, 승인된 change-set
  ...
  shared file, 필수 test와 별도 승인 대상 경계 변경 발견 시 중단 조건을 지정한다.
...
entrypoint와 호출 단위 input/output/domain-error semantics다. 중요한 DTO, Protocol, use case와 lifecycle
capability는 owner 문서에 설명하지만 public constant/function/error 전체를 inventory처럼 열거하지 않는다.
Consumer는 owner leaf module에서 직접 import하고 underscore private symbol에는 의존하지 않는다.

Python shape/signature, helper 추가와 내부 type 정리는 아래 승인 대상 의미를 보존하고 provider와 직접
consumer를 같은 change set에서 검증하면 일반 구현 변경으로 다룬다. 실제로 둘 이상의 구현을 교체하거나
test seam이 필요한 경계에만 Protocol을 둔다. Policy, ordering, limit와 lifecycle outcome은 아래 별도
범주로 분류한다.
...
하나의 변경이 여러 범주에 걸칠 수 있다. 예를 들어 SQL-policy descriptor의 Python shape 정리는 내부
변경일 수 있지만 digest 재료는 policy/compatibility identity이고, HTTP projection은 external wire
...
- External API/wire, persisted/versioned format, policy/compatibility identity, safety/lifecycle invariant,
  ...
  영향으로 별도 승인받는다. 내부 interface 변경이나 다른 범주의 승인을 이 의미 변경의 자동 승인으로
  간주하지 않는다.
- 위 의미를 보존하는 public Python symbol 추가·삭제, shape/signature 변경과 provider/consumer 동시
  refactor는 별도 사용자 승인을 요구하지 않는다. 호환 기간이나 독립 배포 consumer가 실제로 있으면
  ownership/compatibility 영향으로 분류해 승인받는다.
...
- Shared transition artifact라는 사실은 single-writer 요구만 만든다. 그 자체가 의미 변경이나 사용자
  승인 필요성을 뜻하지 않는다.
- 위 경계의 의미를 보존하는 helper, algorithm, lock 구현, public leaf symbol 정리, file move와 오탈자
  수정은 일반 구현 변경이다. Consumer가 의존하지 않는 내부 순서도 module interface가 아니다.
...
- 승인 뒤 owner와 직접 consumer의 동작 문서, 해당되는 external/persisted format, policy,
  ...
  범위를 넘으면 다시 승인받는다.
- Accepted ADR, 실제 provider/consumer behavior, external API/wire, persisted/versioned format,
...
- 보안 parser와 데이터 손실 경로는 최소 테스트 원칙의 예외다. 허용·거부 corpus, 우회 사례와
  property test를 유지한다.
- Architecture test는 모든 허용 symbol을 열거하기보다 DB adapter 직접 접근, source authority 재해석,
  private import와 잘못된 composition처럼 금지 capability를 검사한다.
...
- 구현과 같은 변경에서 canonical owner 문서의 중요한 behavior/entrypoint, external API/wire,
