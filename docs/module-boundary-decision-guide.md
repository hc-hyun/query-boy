...
2. Delivery의 숨은 import를 어느 공개 interface로 돌릴 것인가
...
ledger로 이동했다. 여섯 선택은 모두 현재 구현 baseline이다. 승인된 change-set baseline을
넘어서는 승인 대상 의미 변경은 실제 범주와 영향을 명시해 다시 승인받는다.
...
Control  ──> Assurance verified interface/hash identity
...
| Module interface | Provider가 allowed dependency map과 자기 module 문서에서 다른 module용으로 명시적으로 공개한 Python symbol/callable. 의미는 shape/signature와 호출 단위 input/output/domain-error semantics로 한정한다. |
| Provider | Interface를 제공하는 module |
| Consumer | 그 interface를 사용하는 module |
...
| External API/wire format | HTTP/MCP/CLI처럼 process 밖으로 보이는 요청·응답·오류 형식 |
| Persisted/versioned format | DB나 file에 저장되어 이전 version과도 호환돼야 하는 schema/codec |
| Policy/compatibility identity | Revision, fingerprint, canonical encoding/hash와 allowlist 의미 |
| Safety/lifecycle invariant | 실패·취소·rollback·cleanup 때 반드시 유지할 결과와 순서 |
| Ownership/composition boundary | Private implementation 접근 금지와 concrete 조립 권한 |
| Protected operational procedure | Protected inventory, freeze, DDL, cutover와 rollback의 plan 및 stop/rollback 조건 |
| Operational execution authorization | 실제 protected action의 access, scope, target, stop 조건과 change-record 책임을 확인하는 별도 승인 |
| Evidence/change record | 실행된 사실과 provenance를 append-only/immutable하게 보존하는 기록 |
| Shared transition artifact | 여러 owner가 검토하고 single-writer가 편집하는 공동 file·문서·test |
| Interface test | Provider와 consumer가 같은 module interface를 이해하는지 실행해서 확인하는 test |
| Rolling replica | 서로 다른 code version의 replica가 잠시 함께 실행되는 배포 상태 |

Module interface의 의미는 Python shape/signature와 호출 단위 input/output/domain-error semantics까지다.
Policy, ordering, limit와 lifecycle outcome은 위 별도 범주로 분류한다.

이 문서의 결정은 한 종류가 아니다. `D0`는 safety/lifecycle invariant, `D1`·`D2`·`D4`는
module interface, `D3`는 Python data interface와 별도 wire/persisted/identity 호환 보장, `D5`는
ownership/composition boundary를 강화했다. 서로 다른 범주를 한 단어로 묶지 않는다.
...
`D1-C`/`D5-C`는 방향과 trade-off를 비교하기 위한 대안이며, 선택하면 별도의 정확한 change set
초안을 만든 뒤 다시 승인받는다. `D0-C`/`D2-C`/`D3-C`/`D4-C`는 현재 상태를 유지하고 debt를
보류하는 선택이라 구현 승인이 필요하지 않다.

## Wave 0: 승인 전 read-only prework (완료)

현재 가능한 Wave 0는 경계 변경을 구현하는 단계가 아니다. 다음 read-only prework만
...
- Accepted ADR 또는 승인된 change-set baseline의 의미를 바꾸는 것
- Code, schema, configuration, public type/Protocol 또는 경계 문서의 의미를 변경하는 것
- 권장안이나 TODO 순서를 사용자의 decision 선택으로 간주하는 것

결정 선택과 실행 순서는 Active TODO와 completion ledger에서 다음 ID로 추적한다.

| 경계 결정 | 추적 ID | 상태/공식 시작 gate |
|---|---|---|
...
구현 전 exact follow-up change set을 다시 승인받는다. 반면 `D2-C`/`D3-C`/`D4-C`는
...
startup exception을 보존한다. [Runtime startup rules](modules/runtime/README.md#startup-sequence-and-failure-cleanup)과
...
| 선택 | 내용 | 장점 | 비용·위험 |
|---|---|---|---|
| **D1-A 권장** | Control Plane public interface에 sequence 상한과 verified publish input을 둔다. Delivery는 이 input만 만들고 Control Plane이 Assurance DTO로 변환한다. | 현재 허용 dependency graph를 지키며 Delivery와 Assurance를 분리한다. | Public Python input type과 mapping test가 추가된다. |
| D1-B | `Delivery -> Assurance`를 공식 허용하고 sequence 상한만 Control Plane public interface로 옮긴다. | 변경량이 가장 작다. | Assurance DTO 변경이 Delivery route 변경으로 전파되고 dependency graph가 더 촘촘해진다. |
| D1-C | 전역 `shared_interfaces` package를 새로 만들어 DTO와 상한을 모두 옮긴다. | Import 위치는 한곳이 된다. | 현재 규모에서 owner 없는 공용 창고가 될 위험이 크고 물리 이동 범위가 넓다. |

### D1-A 승인 범위와 구현 결과

Control Plane 공개 interface에 다음 exact public symbol을 추가한다.
...
DB migration은 없다. External API가 같아 mixed-version replica가 함께 있어도 wire/storage
...
이는 악성 in-process code를 막는 sandbox가 아니라 개발 실수를 type 수준에서 차단하는 interface다.
...
확정한다. Mixed-version replica의 external API/persisted format은 동일하게 유지한다.
...
HTTP/MCP API, metadata revision, canonical result encoding과 verified hash identity는 그대로다.
...
검사는 callable 존재를, provider Protocol/mypy/interface test는 sync/async signature를 고정한다.

### 선택지

| 선택 | 내용 | 장점 | 비용·위험 |
|---|---|---|---|
| **D4-A 권장** | 작은 application Protocol은 유지하고 provider module이 Runtime 전용 composite lifecycle Protocol을 추가한다. Runtime은 필수 capability가 모두 있는 객체만 조립한다. | 일반 consumer는 작은 interface를 유지하고 Runtime은 drain/invalidate를 확실히 요구한다. | Runtime injection과 test/custom adapter가 새 Protocol을 만족해야 한다. |
...
- Runtime은 capability를 optional하게 추측하지 않는다. Startup sync/probe와 background task를
  시작하기 전에 required method가 callable인지 composition-time validation하고, mypy와 interface
...
External API/DB format과 data migration은 없다. Python adapter compatibility만 바뀐다.
...
CLI/ownership tests는 console target, help와 `--root` resolve, bootstrap-only config path, evaluate의 exact
...
|---|---|---|---|---|
| D0-A | 없음 | External API 동일 | 최초 error 보존, exactly-once parent cleanup | Code rollback |
| D1-A | 없음 | HTTP/storage 동일 | Private import 제거, verified identity/hash 불변 | Code rollback |
| D2-A | 없음 | Runtime output 동일 | Read consumer interface가 writer capability를 요구·노출하지 않음 | Code rollback |
| D3-A | 없음; golden 호환 검증 완료 | External JSON/revision/hash 동일 | Nested mutation 거부, golden digest/hash 불변 | Code rollback |
| D4-A | 없음 | External API 동일 | Capability 누락 시 ready 전 fail-closed | Code rollback |
...
D4-A  일반 application interface는 작게, Runtime 필수 lifecycle은 명시적으로 유지
...
D3은 보류하고 현재 baseline(D3-C)을 유지한다.
```
...
선택하지 않은 항목은 현재 baseline과 debt 상태를 그대로 유지한다. 일반적인 “이어서 구현”,
“정리” 또는 “권장안대로 잘 해줘”는 이 문서의 경계 변경 승인으로 해석하지 않는다. 이 문서로
...
승인된 경계 변경은 coordinating workstream 하나가 다음 순서로 직렬화한다.
...
7. 전체 dependency/interface audit — 완료 (2026-08-25)

최종 audit에서 당시 interface/boundary 위반이나 migration 필요성은 발견되지 않았다. 이후 `CTRL-06A`의
Control DB schema, replica observation, freshness와 admin response 변경은 2026-08-25 별도 사용자
...
별도 승인을 받아 구현됐고, 기존 persisted/lifecycle 의미를 재현한 `CTRL-09` isolated cross-service Control recovery
...
아직 없어 현재 승인 baseline이나 구현이 아니다. Reader TimeZone/canonical hash 변경은 별도 승인된
...
start/change-set 승인이 아니다.

각 구현 단계는 승인된 change-set baseline, 해당되는 provider/consumer, module 문서, authority,
procedure와 evidence schema/template 및 runnable interface/integration/acceptance test가 함께 통과하는
독립 커밋으로 끝낸다. 실제 evidence/change record는 별도 승인된 operational action을 수행한 뒤에만
append한다. Shared governance artifact는 single-writer로 편집한다. Change-set baseline이 확정된
뒤에만 서로 다른 독립 implementation을 병렬화한다.
...
추가 interface/boundary test는 다음을 고정한다.
...
- Read consumer interface가 writer capability를 요구하거나 노출하지 않음
...
| 관찰 | 현재 위치 |
|---|---|
| Startup enter-failure cleanup 보장 | `src/query_man/app.py` lifespan, [`test_runtime_startup_cleanup.py`](../tests/test_runtime_startup_cleanup.py), [Runtime startup rules](modules/runtime/README.md#startup-sequence-and-failure-cleanup) |
...
| Offline composition 예외 | `src/query_man/assurance_cli.py`, [`test_assurance_cli.py`](../tests/test_assurance_cli.py), [Assurance module](modules/assurance/README.md#offline-cli-surface-and-composition-boundary) |
