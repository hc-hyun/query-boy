# Managed Source Onboarding

Status: Implemented managed onboarding runbook preserved; outside ADR 0025 static first launch
...
onboarding capability를 보존하는 상세 runbook이며 현재 launch에서 신규 database를 hot-add하는
절차가 아니다.

현재 source 추가 요청은 먼저 [source onboarding 안내](source-onboarding.md)와
[extension checklist](source-extension-checklist.md)를 따른다. 이 runbook은 managed mode 활성화가
정확히 별도 승인된 경우에만 사용한다. Repository 절차 승인은 protected environment의 실제 access,
대상, 중단 조건과 변경 기록 책임을 확인한 실행 승인을 대신하지 않는다. 모든 RLS manifest는 현재
publish와 serving에서 차단되므로 아래 역사적 RLS 준비 설명을 허용 절차로 해석하지 않는다.

이 문서의 `generation`, `receipt`, `drift`, `projection` 같은 표현이 낯설다면 먼저
[Query Man 용어 사전](glossary.md)을 읽는다.
