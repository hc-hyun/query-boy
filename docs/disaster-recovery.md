...
3. Database owner/관리자용 `PGHOST`, `PGPORT`, `PGDATABASE`, `PGUSER`와
   ...
   이 script는 control schema와 NOLOGIN writer group/ACL만 적용한다. 네 fixture와 seed를
   만드는 `./scripts/apply-db.sh`는 production migration이 아니다.
...
6. Migration은 immutable revision table의 update/delete trigger를 제거하지 않는다.
...
분리한다. `--no-owner --no-privileges` archive에는 global role, dedicated LOGIN, membership과
ACL이 포함되지 않으므로 이를 database backup과 별도 change record/IaC로 복구한다.

## Restore

1. 격리된 새 `query_man` database에 custom archive를 `pg_restore --no-owner --no-privileges
   --exit-on-error`로 복구한다.
2. 새 database에 `./scripts/apply-control-schema.sh`를 실행해 현재 migration, NOLOGIN writer
   group, ACL과 immutable trigger를 재적용한다.
3. Dedicated runtime LOGIN을 별도로 복구/생성하고 `query_man_control_writer` membership,
   ...
   확인한다.
4. 모든 control table row count, FK와 immutable trigger 존재를 확인한다.
