...
Source마다 semantic metadata와 verified-query record의 완성도가 다르다. 모든 source를 같은
...
- L2: L1 + source와 현재 metadata revision이 일치하는 하나 이상의 verified-query record
...
Canonical-time material처럼 metadata revision 재료가 바뀌는 global transition은 모든 source
metadata revision을 바꾸므로 값이 같아 보이는 verified query도 새 exact revision에서 다시 실행해
새 record로 발행해야 L2가 된다. Metadata revision 재료를 보존하는 SQL-policy-only 전환은 이
문장만으로 metadata revision이나 verified record를 다시 발행하지 않는다.

## Consequences

- L0 source는 자동 catalog만으로 publish할 수 있지만 L1/L2처럼 표시되지 않는다.
- Semantic metadata가 일부만 있는 source는 L0로 판정되므로 L1 minimum을 우회하지 못한다.
- Schema revision이 바뀌면 기존 verified SQL이 있어도 L2가 아니며 새 verified-query record 검증 전
