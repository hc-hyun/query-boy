...
- Default psycopg loader가 interval/time/duplicate·fractional JSON을 무손실로 전달하지 못하고
  ...
  setting과 unversioned collation이 SQL 의미·hash를 흔드는 현재 gap은
  [보류된 result-type 방향](README.md#보류된-방향)에 남긴다. 승인 전에는 현재 encoding을
  lossless/deterministic 범위로 확대해 해석하지 않는다.
