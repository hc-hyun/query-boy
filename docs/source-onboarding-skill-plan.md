...
runbook을 복사해 별도의 진실 원천을 만들지 않는다.
...
| Deterministic helper | 입력 검증, plan binding, 비밀 비출력 전달, admin API 1회 호출, bounded 결과 | 업무 의미 추론, credential 보관, blind retry, policy 완화 |
| Query Man | Operator/source authorization, staging, 암호화, quality gate, atomic publish | Production DB DDL/IAM, human semantic review |
...
- Production artifact를 repository에 저장할지 외부 change system에 저장할지 승인된 retention
  정책을 적용한다.
...
- 별도 source-admin operator token과 reader secret을 helper 내부 secret source에서 읽는다.
...
- Manifest path, canonical hash와 목표 quality `L0`

### Secret inputs

- Plan-only mode는 reader credential이나 operator bearer를 읽거나 존재 확인하지 않는다.
- Executor mode에서는 Codex의 일반 shell/environment가 볼 수 없는 승인된 execution broker가
  helper-owned target config의 operator token과 source-derived reader secret을 전달한다. 같은
  Codex OS identity가 임의 명령으로 provider를 읽을 수 있으면 production executor로 보지
  않는다.
...
- 사용자가 secret을 붙여 넣으면 값을 반복하지 않고 노출된 secret 폐기/회전 안내 후
  onboarding을 중단한다.
- Executor plan은 provider key와 secret version의 일치 여부만 bounded status로 보고 값이나
  일부 문자열을 출력하지 않는다. Secret version이 apply 전에 바뀌면 plan을 폐기한다.

### Evidence

Fixture와 개발 환경에서는 publish 응답의 source ID/generation, replica별 metadata
revision/quality, 검증 결과와 recovery target을 기록할 수 있다. Production endpoint identity,
조직 이름과 retention이 민감할 수 있으므로 repository 기록 여부는 설계 review에서 확정한다.
Ad-hoc 질문과 SQL은 evidence에 저장하지 않는다.
...
Executor mode를 선택할 때의 권고안이다. Plan 단계에서 model이 고를 수 있는 입력은 allowlist
안의 regular manifest path와 미리 구성된 target alias뿐이다. Source ID는 manifest에서 읽고
별도 argument로 덮어쓸 수 없게 한다. Helper-owned target config는 alias를 고정 HTTPS base URL,
operator token provider와 reader-secret provider namespace에 연결한다. Model은 임의 URL,
operator token 위치 또는 secret environment variable 이름을 지정할 수 없다.

Reader secret key는 검증된 source ID를 대문자/underscore 형식으로 canonicalize한
`<SOURCE_ID>_READER_PASSWORD`로 helper가 유도하고 manifest의 `password_env`와 정확히
일치시킨다. 충돌하거나 canonicalize할 수 없는 ID는 거부한다. Symlink/traversal, size 초과
payload, `host_env`/`port_env`, userinfo/query가 든 URL과 redirect도 production executor에서
거부한다. Helper는 generic shell을 실행하지 않고 secret 값은 process/model argument,
plan output 또는 예외에 포함하지 않는다.

Repository Python helper 자체는 같은 OS identity의 Codex로부터 secret을 보호하는 security
boundary가 아니다. Production에서는 target config와 secret provider가 일반 shell에서 접근할
수 없는 별도 principal의 execution broker/container/tool에 있어야 하고, broker는 allowlisted
plan/apply operation만 노출해야 한다. 이 격리를 제공할 수 없으면 Option B를 선택하지 않고
plan-only로 release한다. Repository script는 payload/receipt 검증과 local fixture용 client
역할만 맡는다.

Non-publishing server plan은 source history/active pointer를 바꾸지 않고 resolved
target/source/prior state와 canonical manifest hash, secret provider version에 묶인
...
helper는 target config와 secret provider를 다시 resolve하고 server의 expected-state/idempotency
계약으로 admin API를 정확히 한 번 호출한 뒤 bounded response만 출력한다. 새 dependency 없이
Python 표준 라이브러리와 기존 project dependency를 우선한다.

Sanitized state/idempotency 계약이 없는 현재 API로 production executor를 제공할 수 있는지는
`SKILL-03`과 `SKILL-04`에서 결정한다. 안전한 helper가 없으면 Skill은 plan에서 멈추며 raw
credential을 조립한 `curl`로 fallback하지 않는다.
...
| `SKILL-04` | Executor: `BUILD-01`; plan-only: recorded mode decision and mutation boundary test |
...
- [ ] `DESIGN-03` 공통 Draft→EvidenceReviewed→Planned, plan-only Handoff와 executor
  ...
  condition을 승인한다.
- [ ] `DESIGN-04` Query MCP token, source-admin operator token, reader credential, execution
  broker와 server-only control DSN/encryption key의 분리 모델을 threat review한다.
- [ ] `DESIGN-05` Exact mutation preview에 bind할 helper-owned target, source, canonical manifest
  hash, server-resolved connection identity, prior state, secret provider version, caller 노출
  영향과 approval 만료 조건을 확정한다.
- [ ] `DESIGN-06` Instruction-only와 minimal helper를 secret exposure, idempotency, timeout,
  retry와 유지보수 기준으로 비교해 V1 방식을 선택한다.
- [ ] `DESIGN-07` Fixture와 production의 manifest/evidence 저장 위치, retention, redaction과
  commit 정책을 확정한다.
...
- [ ] `BUILD-01` Production executor를 채택하면 먼저 승인된 non-publishing
  validation/state/plan과 reconciliation 계약 및 race/timeout 회귀를 구현한다.
...
- [ ] `BUILD-04` 선택한 invocation policy와 필요한 dependency만 `agents/openai.yaml`에
  선언한다.
- [ ] `BUILD-05` Option B를 채택하면 target/secret config를 helper가 소유하고 apply 입력을
  single-use plan ID로 제한하며 secret을 argument/output에 노출하지 않는 helper와 unit test를
  구현한다.
...
- Query MCP token은 계속 non-operator이고 별도 operator/reader secret 값이 노출되지 않는다.
...
| Production artifact retention | Pending | Repository와 외부 change/audit system 경계 확인 필요 |
