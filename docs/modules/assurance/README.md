...
marker와 semantic completeness는 Metadata가 직접 검사한다. 이 module은 versioned security corpus,
integration/container test, bounded load와 soak처럼 product safety와 end-to-end lifecycle을 검증하는
repository gate를 소유한다.
...
- Result OID/canonical encoding negative/characterization tests
...
| [`test_integration.py`](../../../tests/test_integration.py) | Real PostgreSQL end-to-end safety와 deterministic smoke |
...
PostgreSQL reader privilege, read-only transaction, AST/relation/function/operator allowlist, timeout,
row/byte/OID limit, cancel/rollback과 revision mismatch를 검사한다. Container gate는 built image의
readiness와 HTTP/MCP public behavior를 확인한다.
...
같은 안전 의미를 보존하는 test helper, fixture ordering, CI job naming과 report formatting은 내부
...
| PostgreSQL integration | fixture SQL/compose, `test_integration.py`, catalog/query lifecycle |
