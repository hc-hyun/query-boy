...
| `app.py` composition/lifespan/probe | Runtime | Concrete `SourceRegistry`를 만들고 reader/writer capability별로 주입한다. Delivery route 동작을 함께 바꾸지 않는다. |
...
`D1-A`/`MOD-04`와 `D2-A`/`MOD-05`는 구현 완료됐고, 나머지 Active TODO가 완료되기 전에는 승인된 target을 현재
