...
| `server.py`, `runtime_config.py`, `operations.py` | Runtime | Domain module은 operations reporting 계약만 소비하며 lifecycle/redaction/health와 managed replica identity/internal observation 의미는 Runtime이 소유한다. Existing public operations snapshot은 replica report를 위해 확장하지 않는다. |
