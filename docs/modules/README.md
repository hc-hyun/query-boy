...
| `tests/test_source_database_corners.py` | Assurance; Source Catalog/Metadata/Guarded Query integration verification | UUID별 disposable PostgreSQL 18 UTF8/SQL_ASCII source에서 wide/untrusted metadata, timezone/direct·hidden-view·domain-type collation/custom-function/encoding·rich scalar/result OID, live definition drift, catalog hard limit, unsupported recovery, multibyte/pool reset, partition/materialized/empty-result 경계를 검증한다. 대부분은 실제 public service 흐름을 통과하고 SQL_ASCII identity, domain/enum OID와 domain `pg_depend`는 raw driver/catalog probe로 특성화하며 필요한 경우 public companion을 둔다. Provider 계약 변경은 각 owner가 먼저 검토하고 test fixture는 Assurance가 쓴다. |
