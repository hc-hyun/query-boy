...
|---|---|
| Source package, reader, budget, semantic overlay | Source Catalog |
...
| 보안 corpus, integration/container, load·soak gate | Assurance |

## 모듈 목록

| Module | 한 문장 책임 | 개요 | 작업별 읽기 |
|---|---|---|---|
| Source Catalog | Git에서 검토한 source package layout·manifest와 budget YAML을 strict validation해 immutable runtime profile로 만든다. | [source-catalog](source-catalog/README.md) | [필요한 code·test](source-catalog/README.md#집중해서-읽을-범위) |
...
| Assurance | 보안·통합·container·load·soak repository gate를 소유한다. | [assurance](assurance/README.md) | [필요한 code·test](assurance/README.md#집중해서-읽을-범위) |
...
- DSN password 같은 secret은 source package나 Git에 저장하지 않고 environment로 resolve한다.
...
Repository safety gates (Assurance ownership) -> Source Catalog(read), Metadata,
                                                 Guarded Query, Delivery, Runtime
```
...
Production concrete implementation 조립은 다음 composition root만 소유한다.
...
source-package pull request와 DBA 작업 handoff를 만들 수 있지만 credential 접근, 파일 변경, DB DDL, API 호출,
...
- Runtime 이외의 위치에서 production concrete implementation을 조립하는 것
...
|---|---|---|
| `source_catalog/models.py`, `source_catalog/registry.py`, `source_catalog/reader_policy.py` | Source Catalog | Immutable source DTO, strict source-package/manifest parser, `SourceReader`, reader connection/session policy |
| `metadata/models.py`, `metadata/catalog.py`, `metadata/service.py`, `metadata/relevance.py`, `metadata/revision.py` | Metadata | Catalog/marker/direct admission/context/revision; persisted metadata store는 없음 |
...
| `runtime/config.py`, `runtime/composition.py`, `runtime/server.py`, `runtime/operations.py`, `runtime/diagnostic_capture.py`, `runtime/operator_shell.py`, `runtime/operator_backend.py` | Runtime | Environment, production composition/lifecycle, safe operations와 local source-package CLI/backend |
| `assurance/__init__.py` | Assurance | Marker-only package; runnable repository gate는 root tests/scripts/CI가 소유 |
...
| `config/sources/`, `config/budget-profiles.yaml` | Source Catalog | Git-reviewed source package authority와 versioned budget |
| `config/access-policies*.yaml` | Delivery | Caller/source/scope policy |
| `config/security-evaluation.yaml` | Assurance | Versioned malicious/invalid input corpus |
...
| `skills/query-man-source-onboarding/` | Source Catalog workflow | Source-package plan handoff; mutation 권한 없음 |
...
- Metadata: `metadata/__init__.py`, `metadata/catalog.py`, `metadata/models.py`,
  `metadata/relevance.py`, `metadata/revision.py`, `metadata/service.py`
...
1. `config/sources/<source-id>/source.yaml`에 source, connection environment key, view-only allowlist,
   기존 budget profile, semantic overlay, provenance와 양의 `view_contract_version`을 추가한다.
2. 같은 폴더의 `views.sql`에 explicit output column, schema-qualified base relation, relation/column
   comment marker, dedicated owner와 exact grant를 둔다. Password·role 생성·base DDL·seed는 두지 않는다.
3. DB/data owner가 exact output과 no-PII를 확인하고 DBA가 별도 승인 아래 traffic 밖에서 적용한다.
   Runtime은 이 SQL을 실행하지 않는다.
4. `qm source validate`, Metadata direct admission과 focused/full repository gate를 통과하고 배포/재시작한다.
   Marker/source/version, semantic completeness, drift, RLS 또는 reader policy 불일치를 fail-closed한다.

Source package schema/version, view contract, metadata revision material, SQL/reader policy나
...
registry, revision, `tests/test_source_view_artifacts.py`, operator CLI, Runtime configuration과
documentation test를 함께 확인한다.
