...
| `tests/test_source_database_corners.py` | Assurance; Source Catalog/Metadata/Guarded Query integration verification | UUID별 disposable PostgreSQL source에서 wide/untrusted metadata, UTC/서울/뉴욕 reader default와 DST/rich scalar/time, live definition drift, catalog hard limit, unsupported driver recovery, multibyte/pool reset, partition/materialized/empty-result 경계를 실제 public service 흐름으로 검증한다. Provider 계약 변경은 각 owner가 먼저 검토하고 test fixture는 Assurance가 쓴다. |
