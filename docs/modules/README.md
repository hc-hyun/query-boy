...
| `delivery/access.py`, `delivery/diagnostics.py`, `delivery/gateway.py`, `delivery/mcp_server.py`, `delivery/http_validation.py` | Delivery | Caller/consent/capture port와 application interface, HTTP/MCP transport와 bounded validation policy를 소유한다. `GatewayService`는 Source Catalog의 `SourceReader`를 소비한다. |
