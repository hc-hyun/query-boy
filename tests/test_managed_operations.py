...
from query_man.managed.runtime import (
...
def test_gateway_usage_groups_terminal_events_by_trusted_dimensions_and_utc_hour() -> None:
    ...
    state = ManagedGatewayUsageRecorder(clock=lambda: current[0])
...
def test_gateway_usage_snapshot_ack_is_retry_safe_and_preserves_new_events() -> None:
    ...
    state = ManagedGatewayUsageRecorder(clock=lambda: observed_at)
    state.record_gateway_usage(
        source_id="source-a",
        budget_profile="standard",
        metadata_revision="revision-1",
...
def test_gateway_usage_reset_clears_pending_without_changing_public_metrics() -> None:
    ...
    recorder = ManagedGatewayUsageRecorder(
    ...
    outstanding = recorder.gateway_usage_report_snapshot()
    assert outstanding is not None
    assert state.snapshot() == public_before

    recorder.reset()
    assert recorder.gateway_usage_report_snapshot() is None
    state.reset()
    ...
    after_reset = recorder.gateway_usage_report_snapshot()
